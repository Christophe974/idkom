'use client';

import { useEffect, useRef, useState } from 'react';
import QRCodeLib from 'qrcode';

interface QRCodeProps {
  url: string;
  size?: number;
  color?: string;
}

export default function QRCode({ url, size = 140, color = '#ffffff' }: QRCodeProps) {
  const [showModal, setShowModal] = useState(false);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const canvasModalRef = useRef<HTMLCanvasElement>(null);

  // Render QR code on canvas
  useEffect(() => {
    if (canvasRef.current) {
      QRCodeLib.toCanvas(canvasRef.current, url, {
        width: size,
        margin: 1,
        color: {
          dark: color.startsWith('#') ? color : `#${color}`,
          light: '#00000000', // transparent background
        },
      }).catch(console.error);
    }
  }, [url, size, color]);

  // Render large QR in modal
  useEffect(() => {
    if (showModal && canvasModalRef.current) {
      QRCodeLib.toCanvas(canvasModalRef.current, url, {
        width: 280,
        margin: 2,
        color: {
          dark: color.startsWith('#') ? color : `#${color}`,
          light: '#00000000',
        },
      }).catch(console.error);
    }
  }, [showModal, url, color]);

  return (
    <>
      <button
        onClick={() => setShowModal(true)}
        className="group relative"
        title="Agrandir le QR code"
      >
        <canvas
          ref={canvasRef}
          className="rounded-lg transition-transform group-hover:scale-105"
          style={{ width: size, height: size }}
        />
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/30 rounded-lg">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-white">
            <path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/>
          </svg>
        </div>
      </button>

      {/* Modal */}
      {showModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4"
          onClick={() => setShowModal(false)}
        >
          <div className="bg-zinc-900 rounded-2xl p-8 max-w-sm w-full text-center border border-zinc-800">
            <p className="text-white font-semibold mb-4">Scannez ou partagez ce QR code</p>
            <canvas
              ref={canvasModalRef}
              className="mx-auto rounded-lg"
              style={{ width: 280, height: 280 }}
            />
            <p className="text-zinc-500 text-xs mt-4 break-all">{url}</p>
            <p className="text-zinc-600 text-xs mt-4">Touchez n&apos;importe où pour fermer</p>
          </div>
        </div>
      )}
    </>
  );
}
