...
export default function ScanPageClient({ token }: Props) {
          ...
          {partner?.logo && (
            <div className="mb-6">
              <div
                ...
                style={partner.logo_bg_color ? { backgroundColor: partner.logo_bg_color } : undefined}
              >
  ...
  // Success - email sent (with partner branding)
  if (success) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ background: '#09090b' }}>
        ...
        <div className="relative z-10 text-center max-w-sm">
          {/* Partner branding on verification page */}
          {partner?.logo && (
            <div className="mb-6">
              <div
              ...
              <p className="text-white font-semibold mt-3">{partner.name}</p>
              {partner.agency && (
                <p className="text-zinc-500 text-sm">{partner.agency}</p>
              )}
            </div>
          )}
          ...
          >
            ...
            <div className="p-8">
              ...
              <div className="text-center mb-6">
                ...
                {partner?.agency && (
                  <p className="text-zinc-400 text-sm mb-3">{partner.agency}</p>
                )}
              ...
              {/* Features - bigger text and icons */}
              <div className="space-y-4 mb-8">
                {[
                  ...
                  { icon: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39', label: 'Lien vers votre site web et réseaux sociaux' },
                  { icon: 'M15 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4m-9-2V7H4v3H1v2h3v3h2v-3h3v-2zm9 4c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4', label: 'Enregistrement du contact en un tap' },
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${accent}20` }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" style={{ color: accent }}>
                        <path d={item.icon} />
                      </svg>
                    </div>
                    <p className="text-zinc-300 text-[15px] leading-snug">{item.label}</p>
                  </div>
                ))}
              </div>
              ...
              <form onSubmit={handleClaim} className="space-y-3">
                <div>
                  <input
                    ...
                    required
                    className="w-full px-4 py-3.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-zinc-500 text-sm focus:outline-none focus:border-white/20 transition-colors"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full py-4 rounded-xl text-white font-semibold text-base flex items-center justify-center gap-2 transition-all duration-300 hover:opacity-90 active:scale-[0.98] disabled:opacity-50"
                  style={{
